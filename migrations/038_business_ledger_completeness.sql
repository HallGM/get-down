-- Migration 038: Wire gig payments and partner drawings into the business account ledger.
--
-- Adds two new arms to account_ledger:
--   Arm 4 — gig payments (payments table) → business account, money in (negative)
--   Arm 5 — drawings (account_transactions where type = 'drawing' on partner accounts)
--           → business account, money out (positive, mirrors partner debit)
--
-- Sign convention (unchanged):
--   positive = money out / owed by the account holder
--   negative = money in / earned by the account holder
