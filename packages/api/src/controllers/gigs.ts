...
router.get("/gigs/:id/card-charges", handle(req => invoicesService.getCardChargesByGig(+req.params.id)));
router.post("/gigs/:id/card-charges", handle(req => invoicesService.addCardChargeToGig(+req.params.id, req.body.invoiceId ?? null, req.body), 201));
router.put("/gigs/:id/card-charges/:chargeId", handle(req => invoicesService.updateCardChargeByGig(+req.params.id, +req.params.chargeId, req.body)));
router.delete("/gigs/:id/card-charges/:chargeId", handle(req => invoicesService.removeCardChargeByGig(+req.params.id, +req.params.chargeId), 204));
